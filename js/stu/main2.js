...
$(document).ready(function () {
  ...
  if (token) {
    ...
    $("#video_selector").change(function () {
      if ($(this)[0].files[0].size / 1024 > (10 * 1024)) {
        alert("大小请控制在20M以内")
      } else {
  ...
  function updateCircle(c, index, item) {
    ...
    for (let i = 0; i < 2; i++) {
      $(skill_text[i]).css({
        "height": circle_seat.radius * (2 / item.name.length ).toFixed(2) + 15 + "px",
        "font-size": circle_seat.radius * (2.8 / getByteLen(item.name)).toFixed(2) + "px",
        "line-height": circle_seat.radius * (2 / item.name.length ).toFixed(2) + 15 + "px",
        "width": (circle_seat.radius * 2).toFixed(2)
      })
    }
  }
